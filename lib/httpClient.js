...
Client.prototype._connect = function () {
  if (this.secure) {
    ...
    this.conn = turbo.connect(this.opts.port, this.opts.hostname)
    /*
    if (this.ipc) {
  ...
  this.conn.read(chunk, function onread (err, data, read) {
    if (err) return onerror(err)
    if (!read) return

    self.resData[self.cer].bytes += read
    self.parser.execute(data, 0, read)
    self.conn.read(chunk, onread)
  })

  function onerror (error) {
    self.emit('connError', error)
    if (!self.destroyed) self._connect()
  }
